...
if(nSel_F>=nfam_F){
	...
	#依据育种值进行窝终选
	ebv_F = ebv[ebv$sex == 'F', c('Id', 'father', 'mother', 'Index')]
	ebv_F = split(ebv_F[, c('Id', 'Index')], list(ebv_F$father, ebv_F$mother), drop = T)
	ebv_F = lapply(ebv_F,function(x) x[order(x[, 2], decreasing = T), ])
	ebv_F = lapply(ebv_F,function(x) x[1:nSel_F, ])  ##每窝选择指定母猪数目
	Select_F = data.table::rbindlist(ebv_F)
	Select_F = Select_F[order(Select_F$Index, decreasing = T), ]
	Select_F = unlist(Select_F[1:nSelected_F, "Id"])
	rm(ebv_F)
}
if(nSel_M>=nfam_M){
	Select_M <- ebv[ebv$Id %in% Phen[Phen$sex == "M", "id"], ][1:nSelected_M, 'Id']
}else{
	ebv_M = ebv[ebv$sex == 'M', c('Id', 'father', 'mother', 'Index')]	
	ebv_M = split(ebv_M[, c('Id', 'Index')], list(ebv_M$father, ebv_M$mother), drop = T)
	ebv_M = lapply(ebv_M,function(x) x[order(x[, 2], decreasing = T), ])
	ebv_M = lapply(ebv_M,function(x) x[1:nSel_M, ])
	Select_M = data.table::rbindlist(ebv_M)
	Select_M = Select_M[order(Select_M$Index, decreasing = T), ]
	Select_M = unlist(Select_M[1:nSelected_M, "Id"])
	rm(ebv_M)
}
