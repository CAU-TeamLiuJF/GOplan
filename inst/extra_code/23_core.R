...
cl <- makeCluster(as.numeric(Ncores), setup_strategy = "sequential")
...
for(pz in Breed){
  ...
  if(paste0(pz,  'sec') %in% names(VAR)){
	a =  get(paste0(pz, 'phe'))
    a$secProb = 1
	if(get(paste0(pz,  'sec')) > 1){for(i in 1:(get(paste0(pz,  'sec'))-1)){ a$secProb = a$secProb + prod(rep(a$cull_sec[1], i)) }}
	a$secPP = c(1/a$secProb)
	if(get(paste0(pz,  'sec')) > 1){for(i in 1:(get(paste0(pz,  'sec'))-1)){ a$secPP = c(a$secPP, prod(rep(a$cull_sec[1], i))/a$secProb) }}
	assign(paste0(pz, 'phe'), a)
	rm(a)
  }
  if('YABd' %in% names(VAR)){
    ABProb <- 1
	if(YABd > 1){for(i in 1:(YABd-1)){ ABProb = ABProb + prod(rep(cull_AB[1], i)) }}
	abPP <- c(1/ABProb)
	if(YABd > 1){for(i in 1:(YABd-1)){ abPP = c(abPP, prod(rep(cull_AB[1], i))/ABProb) }}
  }
  if('YCDs' %in% names(VAR)){
    CDProb <- 1
	if(YCDs > 1){for(i in 1:(YCDs-1)){ CDProb = CDProb + prod(rep(cull_CD[1], i)) }}
	abPP <- c(1/CDProb)
	if(YCDs > 1){for(i in 1:(YCDs-1)){ cdPP = c(cdPP, prod(rep(cull_CD[1], i))/CDProb) }}
  }
  ...
  nSelected_M = ceiling(n_male/Pars$sProb)
  nSelected_F = ceiling(n_female/Pars$dProb)
