...
Phen <- Phen00 # 沿用之前的
    ...
    #-------------------------------------------------------------------------------------------------------------
		...
		if (Mate == 'rand') {
		  ...
		  crossplan$id_b = as.character(crossplan$id_b)
		  crossplan$id_s = as.character(crossplan$id_s)
		  dfInb[g-(6+Yd)] <- crossplan[nrow(crossplan), 'id_s']
		  crossplan <- as.matrix(crossplan[-nrow(crossplan), ])
		} else if (Mate == 'HM') {
			mateRes = HM(path = 'plans.txt', maxF = maxF, Sor = Sor)
	...
	if (Method != "blup") {
      ...
      Geno = rbind(cbind(as.data.frame(matrix(0,nrow=length(ID),ncol=1)),
        	       ...
        	       Geno2)
      Geno = TraceGeno(ped = Ped, Geno = Geno, n = nGenoG, id = famselect@id)    ##近4代基因型
      Geno2 = Geno
	  if(Method == 'ssgblup'){
