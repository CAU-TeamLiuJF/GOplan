...
cull_AB <- cull_AB[1]
	ABProb <- 1
	if(YABd > 1){for(i in 1:(YABd-1)){ ABProb = ABProb + pow(cull_AB, i) }}
	abPP <- c(1/ABProb)
	if(YABd > 1){for(i in 1:(YABd-1)){ abPP = c(abPP, pow(cull_AB, i)/ABProb) }}
	ppp[['abPP']] = abPP
cull_CD <- cull_CD[1]
	CDProb <- 1
	if(YCDs > 1){for(i in 1:(YCDs-1)){ CDProb = CDProb + pow(cull_CD, i) }}
	cdPP <- c(1/CDProb)
	if(YCDs > 1){for(i in 1:(YCDs-1)){ cdPP = c(cdPP, pow(cull_CD, i)/CDProb) }}
	ppp[['cdPP']] = cdPP
for(m in Breed){
	b = get(paste0(m, 'phe'))
	b$secProb = 1              ##total proportion of all age classes
	if(get(paste0(m,  'sec')) > 1){for(i in 1:(get(paste0(m, 'sec'))-1)){
		b$secProb = b$secProb + pow(b$cull_sec[1], i)}}
