...
CoreRes <- function(var_name = var_name){
	...
	for (phe in Trait) {
	  ...
	  out_varg[phe] <- apply(Data[, -1], 1, mean)
	  out_acc[phe] <- apply(Data1[, -1], 1, mean)
	}
	...
	mean_G <- data.frame(matrix(NA, ncol = length(Trait), nrow = nrow(get(Trait[[1]]))))
	names(mean_G) <- Trait
	n <- length(Trait)
	for (i in 1:n) {
	  a <- get(Trait[[i]])
	...
	for(t in c(1:Time)){
	  ...
	  #各性状第Time年预期表型进展
	  Phenos <- c(t(out_results[["pop0_G"]][t, ] - out_results[["pop0_G"]][1, ]))
	  names(Phenos) <- names(mean_G_pop0)
	  ##计算当前表型下各个经济参数的值
	  for(j in cost_items){
		aa <- get(j)
